...
source("Codes_Fat/01_ML_Classified_functions.R")
...
# Update target variables to binary versions
binary_tar_var <- c("Pancreas_PDFF_binary", "Liver_PDFF_binary", 
                    "Muscle_fat_binary", "Pericardial_fat_binary")

# Modified main loop
for (i in c(1:length(binary_tar_var))) {
  
  tar_col_tem <- match(binary_tar_var[i], names(dat_classfied))
  dat_tem <- dat_classfied[, c(tar_col_tem, 2:5, 10:(ncol(dat_classfied)-4))]
  names(dat_tem)[1] <- "target"
  dat_tem$target <- factor(dat_tem$target, levels = c(0, 1), labels = c("Low", "High"))
  ...
  cat("Target variable:", binary_tar_var[i], "\n")
  
  # Prepare data containing only proteins
  protein_data <- dat_tem[, c("target", protein_names)]
  
  # Data preprocessing
  protein_ns_dat <- prepare_lasso_data_classification(data = protein_data, 
  ...
  lasso_results <- perform_lasso_selection_classification(protein_processed)
  ...
  for (model_type in 1:3) {
    ...
    cat("Processing target variable:", binary_tar_var[i], "Model type:", model_type, "\n")
    ...
    # Train models
    models <- train_classification_models(train_data)
    
    # Model comparison
    comparison_results <- compare_classification_models(models, test_data)
    
    # Ensemble learning
    ensemble_results <- create_classification_ensemble(
    ...
    shap_analysis <- analyze_shap_values_classification(models, test_data, top_n = 50)
    
    # Protein score (only for Model 1)
    if (model_type == 1) {
      # Build protein score using fixed LASSO coefficients
      lasso_coef_fixed <- lasso_fixed_features_classification(data = train_data, features = target_proteins)
      train_data$Protein_Score <- build_protein_score_classification(train_data, lasso_coef_fixed)
      test_data$Protein_Score <- build_protein_score_classification(test_data, lasso_coef_fixed)
      
      protein_score_train <- evaluate_protein_score_classification(train_data$Protein_Score, train_data$target)
      protein_score_test <- evaluate_protein_score_classification(test_data$Protein_Score, test_data$target)
    } else {
    ...
    all_models_results[[model_type]] <- list(
      ...
      selected_proteins = target_proteins,
      models = models,
  ...
  save(all_models_results,
       file = paste0("Process_Data/Fat_Dis/Classification/", tar_var[i], "_Hierarchical_Classification_Models.RData"))
  # Print summary report
  print_classification_model_summary(all_models_results, tar_var[i])
}
