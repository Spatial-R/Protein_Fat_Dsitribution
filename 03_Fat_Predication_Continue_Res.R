...
dat.ana.2 <- dat.ana.1[, match(c("eid", tar_var_name), names(dat.ana.1))]
...
tar_var_dat <- batch_remove_effects(dat.ana.u.1,outcome_vars= tar_var,covariate_vars = c("age", "sex"))
names(tar_var_dat) <- gsub("_residual","",names(tar_var_dat))

# Modified main loop
for (i in c(1:length(tar_var))) {
  
  dat_tem <- dat.ana.u.1[, c(2:5, 10:ncol(dat.ana.u.1))]
  ...
  save(all_models_results,
       file = paste0("Continues_Res/", tar_var[i], "_Hierarchical_Models.RData"))
