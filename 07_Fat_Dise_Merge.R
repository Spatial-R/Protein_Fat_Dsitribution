...
View(dat_tem <- filter(dis_reg,var %in% tar_var & model_type == 2 & p_ind < 0.05))
write.csv(dat_tem,file = "/Users/zhangbing/Documents/Tmp/Fat_Dis_Sig_1.csv",row.names = F)
write.csv(dis_reg,file = "/Users/zhangbing/Documents/Tmp/Fat_Dis_Full_0.csv",row.names = F)
...
# Set color scheme
var_colors <- c("Muscle FI" = "#E41A1C", 
                "Liver PDFF" = "#377EB8",
                "Pancreas PDFF" = "#4DAF4A", 
                "PFA" = "#984EA3")
...
combined_plot <- plot_grid(p2,plot_grid(p4,p7,ncol = 1,labels = c("B","C")),ncol = 2,
                           labels = c("A",""),
